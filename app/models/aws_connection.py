...
class AWSConnection(Base):
    """
    Represents a user's AWS account connection to CloudSentinel.
    
    Flow:
    1. User deploys CloudFormation to create IAM role in their account
    2. User registers the role ARN here
    3. CloudSentinel uses STS AssumeRole to get temp credentials
    ...
    # Format: arn:aws:iam::123456789012:role/CloudSentinelReadOnly
    role_arn = Column(String(2048), nullable=False)
    
    # external_id: Unique identifier for confused deputy prevention
    # Generated by CloudSentinel, user must add to role trust policy
    external_id = Column(String(64), nullable=False, default=lambda: f"cs-{secrets.token_hex(16)}")
