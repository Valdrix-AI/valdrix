...
class AWSConnection(Base):
    """
    Represents a user's AWS account connection to Valdrics.

    Flow:
    1. User deploys CloudFormation to create IAM role in their account
    2. User registers the role ARN here
    3. Valdrics uses STS AssumeRole to get short-lived credentials
    ...
    # Format: arn:aws:iam::123456789012:role/ValdricsReadOnly
    # ENCRYPTED at rest
    role_arn: Mapped[str] = mapped_column(
        StringEncryptedType(String(2048), get_encryption_key, AesEngine, "pkcs5"),
        nullable=False,
    )

    # external_id: Unique identifier for confused deputy prevention
    # Generated by Valdrics, user must add to role trust policy
    # ENCRYPTED at rest
    external_id: Mapped[str] = mapped_column(
        StringEncryptedType(String(64), get_encryption_key, AesEngine, "pkcs5"),
        nullable=False,
