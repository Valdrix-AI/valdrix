...
class PlatformConnection(Base):
    """
    Cloud+ connector for internal platform/shared-services spend feeds.

    v1 is feed-based (manual/csv) and intentionally generic to support:
    - internal chargeback ledgers
    - shared Kubernetes/platform costs
    - shared services and platform tooling costs not captured in cloud provider bills
    """

    __tablename__ = "platform_connections"
    __table_args__ = (
        UniqueConstraint(
            "tenant_id", "vendor", "name", name="uq_tenant_platform_vendor_name"
        ),
    )
    ...
    tenant: Mapped["Tenant"] = relationship(
        "Tenant", back_populates="platform_connections"
    )
