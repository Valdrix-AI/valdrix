        ...
        **kwargs: Any,
        ...
        try:
            async with self._get_client(
                session, "ecr", region, credentials, config=config
            ) as ecr:
                ...
                async for repo_page in repo_paginator.paginate():
                    for repo in repo_page.get("repositories", []):
                        ...
                        async for img_page in img_paginator.paginate(
                            repositoryName=name
                        ):
                            for img in img_page.get("imageDetails", []):
                                if "imageTags" not in img:
                                    ...
                                    if pushed_at and pushed_at < cutoff:
                                        size_gb = img.get("imageSizeInBytes", 0) / (
                                            1024**3
                                        )
                                        monthly_cost = (
                                            size_gb
                                            * PricingService.get_hourly_rate(
                                                provider="aws",
                                                resource_type="ecr",
                                                region=region,
                                            )
                                            * 730
                                        )  # GB-month pricing logic simplified
                                        zombies.append(
                                            {
