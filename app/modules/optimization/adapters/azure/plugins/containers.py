        ...
        try:
            ...
            async for cluster in client.managed_clusters.list():
                ...
                if total_nodes == 0:
                    ...
                    estimated_cost = 73.0 if cluster.sku and cluster.sku.tier == "Paid" else 0.0
        ...
        try:
            ...
            async for plan in client.app_service_plans.list():
                ...
                if len(apps) == 0 and plan.sku and plan.sku.tier != "Free":
                    ...
                    tier_costs = {"Basic": 55, "Standard": 73, "Premium": 146, "PremiumV2": 180}
                    ...
                    zombies.append({
                        ...
                        "sku": f"{plan.sku.tier} ({plan.sku.name})" if plan.sku else "Unknown",
                        "monthly_cost": round(estimated_cost, 2),
