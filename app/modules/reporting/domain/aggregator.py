...
class CostAggregator:
    ...
    async def get_summary(
            ...
            schema_records.append(
                SchemaCostRecord(
                    date=datetime.combine(
                        r.recorded_at, datetime.min.time(), tzinfo=timezone.utc
                    ),
                    amount=r.cost_usd,
                    service=r.service,
                    region=r.region,
        ...
        return CloudUsageSummary(
            ...
            metadata={
                ...
                "summary": "Breakdown/records are partial"
                if is_truncated
                else "Full data",
            },
        )
    ...
    async def get_dashboard_summary(
        ...
        if db.bind.dialect.name != "sqlite":
            await db.execute(
                text(f"SET LOCAL statement_timeout TO {STATEMENT_TIMEOUT_MS}")
            )
    ...
    async def get_canonical_data_quality(
        ...
        mapped_pct = (
            (mapped_records / total_records * 100) if total_records > 0 else 0.0
        )
        target_pct = 99.0
    ...
    async def get_governance_report(
        ...
        insights = await engine.get_unallocated_analysis(
        ...
        return {
            ...
            "message": "High unallocated spend detected (>10%)."
            if untagged_percent > 10
            else "Cost attribution is within healthy bounds.",
            "recommendation": "High unallocated spend detected. Implement attribution rules to improve visibility."
            if untagged_percent > 10
            else None,
        }
