...
class GCPUsageAnalyzer:
    ...
    def find_idle_vms(
        self, days: int = 7, cpu_threshold: float = 5.0
    ) -> List[Dict[str, Any]]:
        """
        ...
        for resource_id, records in self._resource_costs.items():
            ...
            total_cpu_hours = sum(
                float(r.get("usage_amount", 0) or 0)
                for r in compute_records
                if any(
                    x in r.get("sku_description", "").lower()
                    for x in ["cpu", "core", "vcpus"]
                )
            )
            total_network = sum(
                float(r.get("usage_amount", 0) or 0)
            ...
            is_gpu = any(
                "gpu" in r.get("sku_description", "").lower() for r in compute_records
            )
    ...
    def find_idle_cloud_sql(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for resource_id, records in self._resource_costs.items():
            sql_records = [
                r for r in records if "cloud sql" in r.get("service", "").lower()
            ]
    ...
    def find_empty_gke_clusters(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        for resource_id, records in self._resource_costs.items():
            ...
            control_plane_cost = sum(
                float(r.get("cost", 0) or 0)
                for r in gke_records
    ...
    def find_idle_cloud_functions(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        for resource_id, records in self._resource_costs.items():
            function_records = [
                r for r in records if "cloud functions" in r.get("service", "").lower()
            ]
    ...
    def find_idle_cloud_run(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        for resource_id, records in self._resource_costs.items():
            run_records = [
                r for r in records if "cloud run" in r.get("service", "").lower()
            ]
    ...
    def find_old_snapshots(self, age_days: int = 90) -> List[Dict[str, Any]]:
        ...
        for resource_id, records in self._resource_costs.items():
            snapshot_records = [
                r for r in records if "snapshot" in r.get("sku_description", "").lower()
            ]
            ...
            oldest_record = min(
                snapshot_records, key=lambda x: x.get("usage_start_time", now)
            )
