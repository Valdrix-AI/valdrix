...
class HybridConnectionService:
    ...
    async def list_connections(self, tenant_id: UUID) -> list[HybridConnection]:
    ...
    async def verify_connection(
        ...
        failure_message = adapter.last_error or "Failed to validate hybrid connector."
        connection.error_message = None if success else failure_message
        await self.db.commit()

        if success:
            return {"status": "success", "message": "Hybrid connection verified."}
