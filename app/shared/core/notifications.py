...
class NotificationDispatcher:
    ...
    @staticmethod
    async def notify_zombies(zombies: Dict[str, Any], estimated_savings: float = 0.0) -> None:
    ...
    async def notify_budget_alert(
        current_spend: float, budget_limit: float, percent_used: float
    ) -> None:
    ...
    async def notify_remediation_completed(
        tenant_id: str, resource_id: str, action: str, savings: float
    ) -> None:
