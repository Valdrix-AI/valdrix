...
STUCK_JOB_COUNT = Gauge(
    "stuck_job_count",
    "Current number of jobs detected as stuck in scheduler sweeps"
)
...
MEMORY_USAGE_BYTES = Gauge(
    "valdrix_ops_memory_usage_bytes",
    "Current memory usage in bytes",
    ["process"]
)

CPU_USAGE_PERCENT = Gauge(
    "valdrix_ops_cpu_usage_percent",
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if "db" in operation_name.lower():
                    DB_QUERY_DURATION.labels(operation_type=f"{operation_name}_error").observe(duration)
