...
|---|---|---|---|---|
| AWS Cost Explorer API calls are disabled in platform paths | evidence-backed | `app/shared/adapters/factory.py`, `cloudformation/valdrics-role.yaml` | `tests/unit/core/test_competitive_guardrails.py::test_no_cost_explorer_client_usage_in_app_code`, `tests/unit/core/test_competitive_guardrails.py::test_aws_role_template_excludes_cost_explorer_actions` | CE is blocked by design; CUR is required for ingestion. |
| AWS customer IAM template excludes `ce:*` permissions | evidence-backed | `cloudformation/valdrics-role.yaml` | `tests/unit/core/test_competitive_guardrails.py::test_aws_role_template_excludes_cost_explorer_actions` | Template comments also document this guardrail. |
