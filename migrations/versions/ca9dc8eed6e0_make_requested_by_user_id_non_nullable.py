...
def upgrade() -> None:
    ...
    op.drop_index(op.f('ix_audit_tenant_time'), table_name='audit_logs')
    op.drop_index(op.f('ix_audit_type_time'), table_name='audit_logs')
    op.drop_table('audit_logs')
    op.create_unique_constraint('uq_tenant_azure_subscription', 'azure_connections', ['tenant_id', 'subscription_id'])
    op.add_column('background_jobs', sa.Column('priority', sa.Integer(), nullable=False))
    ...
    op.add_column('remediation_settings', sa.Column('hard_cap_enabled', sa.Boolean(), nullable=False))
    op.add_column('remediation_settings', sa.Column('monthly_hard_cap_usd', sa.Numeric(precision=12, scale=2), nullable=False))
