...
async def analyze_inventory():
    ...
    async with engine.connect() as conn:
        ...
        partitions = [
            t.name
            for t in all_tables
            if any(x in t.name for x in ["_2026", "_2027", "_p2026", "_p2027"])
        ]
        ...
        parents = {}
        for p in partitions:
            if "cost_records" in p:
                parents.setdefault("cost_records", []).append(p)
            elif "audit_logs" in p:
                parents.setdefault("audit_logs", []).append(p)
            else:
                parents.setdefault("other", []).append(p)
