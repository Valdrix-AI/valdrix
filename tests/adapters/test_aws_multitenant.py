...
async def test_get_daily_costs_success(adapter):
                ...
                "Groups": [
                    {
                        "Keys": ["S3"],
                        "Metrics": {
                            "AmortizedCost": {"Amount": "100.0", "Unit": "USD"}
                        },
                    }
                ],
            }
    ...
    with patch.object(adapter, "session", mock_session):
        results = await adapter.get_daily_costs(
                ...
                "ResultsByTime": [
                    {
                        ...
                        "Groups": [
                            {
                                "Keys": ["S3"],
                                "Metrics": {"AmortizedCost": {"Amount": "100"}},
                            }
                        ],
                    }
                ],
                ...
                "ResultsByTime": [
                    {
                        ...
                        "Groups": [
                            {
                                "Keys": ["EC2"],
                                "Metrics": {"AmortizedCost": {"Amount": "50"}},
                            }
                        ],
                    }
                ],
...
async def test_get_daily_costs_error_handling(adapter):
    ...
    mock_ce.get_cost_and_usage.side_effect = ClientError(
        {"Error": {"Code": "AccessDenied", "Message": "Boom"}}, "get_cost_and_usage"
    )
...
async def test_get_credentials_retries_transient_errors(adapter):
    ...
    with (
        patch.object(adapter, "session", mock_session),
...
async def test_get_cost_and_usage_normalizes_records(adapter):
    ...
    with patch.object(
        adapter, "get_daily_costs", new_callable=AsyncMock, return_value=summary
    ) as mock_get:
        ...
        "ResultsByTime": [
            {
                ...
                "Groups": [
                    {"Keys": ["S3"], "Metrics": {"AmortizedCost": {"Amount": "1.0"}}}
                ],
            }
        ],
        "NextPageToken": "more",
