...
class TestAnalyze:
    ...
    async def test_invokes_llm_with_cost_data(self, mock_usage_summary, mock_db):
        mock_llm = MagicMock(spec=BaseChatModel)
        mock_llm.ainvoke = AsyncMock(return_value=AIMessage(content='{"summary":"ok","anomalies":[],"recommendations":[],"estimated_total_savings":0}'))
        ...
        with patch(patch_paths[0], new_callable=AsyncMock) as m_reserve, \
            ...
            _ = await analyzer.analyze(mock_usage_summary, tenant_id=mock_usage_summary.tenant_id)
    ...
    async def test_returns_parsed_result(self, mock_usage_summary, mock_db):
        ...
        with patch("app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve", AsyncMock(return_value=Decimal("0.05"))), \
             patch("app.shared.llm.analyzer.LLMBudgetManager.record_usage", AsyncMock()), \
             patch("app.shared.llm.analyzer.SymbolicForecaster.forecast", AsyncMock(return_value={"total_forecasted_cost": 120})), \
            ...
            result = await analyzer.analyze(mock_usage_summary, tenant_id=mock_usage_summary.tenant_id)
    ...
    async def test_handles_markdown_wrapped_json(self, mock_usage_summary, mock_db):
        ...
        with patch("app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve", AsyncMock(return_value=Decimal("0"))), \
             patch("app.shared.llm.analyzer.LLMBudgetManager.record_usage", AsyncMock()), \
             patch("app.shared.llm.analyzer.SymbolicForecaster.forecast", AsyncMock(return_value={})), \
             patch("app.shared.llm.analyzer.get_cache_service") as m_cache:
    ...
    async def test_handles_empty_cost_data(self, mock_usage_summary, mock_db):
        mock_llm = MagicMock(spec=BaseChatModel)
        mock_llm.ainvoke = AsyncMock(return_value=AIMessage(content='{"insights":[],"anomalies":[],"recommendations":[],"summary":"ok"}'))
