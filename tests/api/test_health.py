...
async def test_health_db_failure(ac: AsyncClient):
    ...
    with patch(
        "sqlalchemy.ext.asyncio.AsyncSession.execute", side_effect=Exception("DB Down")
    ):
        # Mock the service directly to be sure
        with patch(
            "app.shared.health.HealthService.check_database",
            return_value=(False, {"error": "DB Down"}),
        ):
            response = await ac.get("/health")
