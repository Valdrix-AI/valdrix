...
async def test_execute_action_ec2_variations(remediation_service):
    ...
    with patch.object(
        remediation_service, "_get_client", new_callable=AsyncMock
    ) as mock_get_client:
        mock_ec2 = AsyncMock()
...
async def test_execute_action_s3_and_ecr(remediation_service):
    ...
    with patch.object(
        ...
        await remediation_service._execute_action(
            "my-bucket", RemediationAction.DELETE_S3_BUCKET
        )
        ...
        await remediation_service._execute_action(
            "repo@sha256:123", RemediationAction.DELETE_ECR_IMAGE
        )
...
async def test_execute_action_redshift_and_elb(remediation_service):
    ...
    with patch.object(
        ...
        await remediation_service._execute_action(
            "cluster-123", RemediationAction.DELETE_REDSHIFT_CLUSTER
        )
        mock_redshift.delete_cluster.assert_called_with(
            ClusterIdentifier="cluster-123", SkipFinalClusterSnapshot=True
        )

        # Mock ELB
        mock_get_client.return_value.__aenter__.return_value = mock_elb
        await remediation_service._execute_action(
            "arn:elb", RemediationAction.DELETE_LOAD_BALANCER
        )
...
async def test_execute_action_sagemaker(remediation_service):
    ...
    with patch.object(
        ...
        await remediation_service._execute_action(
            "endpoint-123", RemediationAction.DELETE_SAGEMAKER_ENDPOINT
        )
        ...
        mock_sagemaker.delete_endpoint_config.assert_called_with(
            EndpointConfigName="endpoint-123"
        )
...
async def test_create_backups(remediation_service):
    ...
    with patch.object(
        ...
        mock_redshift = AsyncMock()
        mock_get_client.return_value.__aenter__.return_value = mock_redshift
        snapshot_id = await remediation_service._create_redshift_backup(
            "cluster-123", 14
        )
...
async def test_aws_action_error_handling(remediation_service):
    ...
    with patch.object(
        ...
        with pytest.raises(ClientError):
            await remediation_service._execute_action(
                "vol-123", RemediationAction.DELETE_VOLUME
            )
