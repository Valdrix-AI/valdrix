...
async def test_savings_processor_executes_autonomous_ready(db):
    ...
    with patch("app.modules.optimization.domain.remediation.RemediationService", autospec=True) as mock_service_cls:
        ...
        assert kwargs["resource_id"] == "i-999"
        assert kwargs["action"] == RemediationAction.STOP_INSTANCE
        assert kwargs["estimated_savings"] == 45.0
        
        mock_service.approve.assert_called_once()
        mock_service.execute.assert_called_once_with(mock_request.id, tenant_id, bypass_grace_period=False)
...
@pytest.mark.asyncio
async def test_savings_processor_does_not_auto_execute_destructive_actions(db):
    tenant_id = uuid4()
    rec = FinOpsRecommendation(
