    ...
    return CloudUsageSummary(
        ...
        records=[
            ...
            CostRecord(
                ...
                service="EBS",
                region="us-east-1",
                tags={"ResourceId": "vol-1234567890abcdef0", "Name": "unused-volume-01"}
            )
        ],
...
class TestZombieDetectionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_zombie_detection_end_to_end(self, mock_llm, sample_cloud_usage, test_tenant, db):
        ...
        with patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', new_callable=AsyncMock) as mock_reserve, \
            ...
            mock_invoke.return_value = ('{"summary": "Found 2 zombies", "anomalies": [{"resource": "idle-server-01", "issue": "Low utilization EC2 instance", "cost_impact": "$450", "severity": "high"}, {"resource": "unused-volume-01", "issue": "Unused EBS volume", "cost_impact": "$25", "severity": "medium"}], "recommendations": ["Terminate idle-server-01", "Delete unused-volume-01"], "estimated_total_savings": 475}', {"token_usage": {"prompt_tokens": 500, "completion_tokens": 500}})

            mock_process.return_value = {
                "insights": ["Found 2 zombies"],
                "anomalies": [
                    {"resource": "idle-server-01", "issue": "Low utilization EC2 instance", "cost_impact": "$450", "severity": "high"},
                    {"resource": "unused-volume-01", "issue": "Unused EBS volume", "cost_impact": "$25", "severity": "medium"}
                ],
                "recommendations": ["Terminate idle-server-01", "Delete unused-volume-01"],
            ...
            result = await analyzer.analyze(sample_cloud_usage, tenant_id=test_tenant.id, db=db)
            ...
            assert any("unused-volume-01" in anomaly["resource"] for anomaly in anomalies)
    ...
    @pytest.mark.asyncio
    async def test_zombie_detection_with_budget_exceeded(self, mock_llm, sample_cloud_usage, test_tenant, db):
        ...
        with patch.object(analyzer, '_check_cache_and_delta', return_value=(None, False)), \
            ...
            with pytest.raises(BudgetExceededError):
                await analyzer.analyze(sample_cloud_usage, tenant_id=test_tenant.id, db=db)

    @pytest.mark.asyncio
    async def test_zombie_detection_with_cache_hit(self, mock_llm, sample_cloud_usage, test_tenant):
...
class TestRemediationIntegration:
    ...
    async def test_remediation_workflow_end_to_end(self, db, test_tenant):
        ...
        with patch('app.modules.optimization.domain.remediation.RemediationService._get_client') as mock_get_client, \
            ...
            mock_ec2_client.stop_instances.return_value = {"StoppingInstances": [{"InstanceId": "i-idle123"}]}

            # Execute remediation
            service = RemediationService(db)
            await service.execute(remediation_request.id, test_tenant.id, bypass_grace_period=True)
            ...
            mock_ec2_client.stop_instances.assert_called_once_with(InstanceIds=["i-idle123"])
...
class TestTenantIsolationIntegration:
    ...
    async def test_tenant_data_isolation_in_analysis(self, mock_llm, db):
        ...
        with patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', return_value=Decimal("1.50")), \
             patch.object(analyzer, '_check_cache_and_delta', return_value=(None, False)), \
             patch('app.shared.llm.analyzer.LLMGuardrails.sanitize_input') as mock_sanitize, \
             patch('app.shared.llm.analyzer.SymbolicForecaster.forecast') as mock_forecast, \
             patch.object(analyzer, '_setup_client_and_usage') as mock_setup, \
            ...
            mock_invoke.return_value = ('{"summary": "Tenant 1 analysis", "anomalies": [{"resource": "i-tenant1", "issue": "Tenant 1 issue"}]}', {"token_usage": {"prompt_tokens": 500, "completion_tokens": 500}})
    ...
    async def test_remediation_tenant_isolation(self, db):
        ...
        with patch('app.modules.optimization.domain.remediation.RemediationService._get_client') as mock_get_client:
            mock_ec2_client = AsyncMock()
            mock_get_client.return_value.__aenter__.return_value = mock_ec2_client
            mock_ec2_client.stop_instances.return_value = {"StoppingInstances": [{"InstanceId": "mock"}]}
            ...
            assert request2.status == RemediationStatus.PENDING  # Should remain unchanged
...
class TestPerformanceIntegration:
    ...
    async def test_concurrent_zombie_detection(self, mock_llm):
        ...
        with patch.object(analyzer, 'analyze', side_effect=mock_analyze):
            ...
            tasks = [analyzer.analyze(usage, tenant_id=usage.tenant_id) for usage in usage_summaries]
    ...
    async def test_large_dataset_zombie_detection(self, mock_llm):
        ...
        large_records = []
        for i in range(1000):
            large_records.append(CostRecord(
        ...
        with patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', return_value=Decimal("1.50")), \
            ...
            mock_forecast.return_value = {"forecast": "test"}
            mock_setup.return_value = (None, "groq", "llama-3.3-70b-versatile", None)
            mock_invoke.return_value = ('{"summary": "Large dataset analysis"}', {"token_usage": {"prompt_tokens": 500, "completion_tokens": 500}})
            mock_process.return_value = {"result": "success", "anomalies": []}

            # Execute analysis
            result = await analyzer.analyze(large_usage, tenant_id=large_usage.tenant_id)
            ...
            assert duration < 5.0, f"Analysis too slow: {duration:.2f}s for 1000 records"
...
class TestErrorHandlingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_zombie_detection_llm_failure_fallback(self, mock_llm, sample_cloud_usage, test_tenant):
        ...
        with patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', return_value=Decimal("1.50")), \
             ...
             patch.object(analyzer, '_invoke_llm', return_value=("LLM service unavailable", {})), \
             patch.object(analyzer, '_process_analysis_results') as mock_process:
    ...
    async def test_remediation_partial_failure_recovery(self, db, test_tenant):
        ...
        with patch('app.modules.optimization.domain.remediation.RemediationService._get_client') as mock_get_client:
            ...
            for request in requests:
                await service.execute(request.id, test_tenant.id, bypass_grace_period=True)
