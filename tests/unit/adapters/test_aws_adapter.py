...
def mock_session():
    ...
    with patch("app.shared.adapters.aws_multitenant.aioboto3.Session") as mock_session_cls:
...
def aws_adapter(mock_session):
    ...
    with patch("app.shared.adapters.aws_multitenant.MultiTenantAWSAdapter.get_credentials", new_callable=AsyncMock) as mock_get_creds:
...
async def test_aws_adapter_verify_connection(aws_adapter):
    ...
    mock_client.get_cost_and_usage.return_value = {"ResultsByTime": mock_results_with_groups}
...
async def test_aws_adapter_stream_cost_and_usage(aws_adapter):
    ...
    mock_results_with_groups = [
        {"TimePeriod": {"Start": "2026-01-01", "End": "2026-01-02"}, 
         "Groups": [{"Keys": ["Total"], "Metrics": {"AmortizedCost": {"Amount": "15.5", "Unit": "USD"}}}]}
    ]
    mock_client.get_cost_and_usage.return_value = {"ResultsByTime": mock_results_with_groups}
