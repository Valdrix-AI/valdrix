...
async def test_search_resources_success(mock_connection):
    ...
    mock_response = {
        "Resources": [
            {"Arn": "arn:aws:ec2:us-east-1:123456789012:instance/i-123456", "Service": "ec2", "ResourceType": "instance", "Region": "us-east-1"},
            {"Arn": "arn:aws:rds:us-east-1:123456789012:db:mydb", "Service": "rds", "ResourceType": "db", "Region": "us-east-1"}
        ]
    }
...
async def test_is_enabled_true(mock_connection):
    ...
    mock_client.__aenter__ = AsyncMock(return_value=mock_client)
    mock_client.__aexit__ = AsyncMock(return_value=None)
    mock_client.list_views = AsyncMock(return_value={"Views": ["arn:aws:resource-explorer-2:us-east-1:123456789012:view/default"]})
