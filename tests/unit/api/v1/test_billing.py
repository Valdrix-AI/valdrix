...
async def test_create_checkout_success(mock_settings, mock_billing_service_class, mock_db, mock_user):
    ...
    mock_billing.create_checkout_session = AsyncMock(return_value={
...
async def test_handle_webhook_success(mock_settings, mock_retry_class, mock_handler_class, mock_db):
    ...
    request.body.return_value = b'{"event": "charge.success", "data": {"reference": "ref-123"}}'
...
async def test_update_pricing_plan_success(mock_db, mock_user):
    ...
    response = await update_pricing_plan(MagicMock(), "starter", plan_req, mock_user, mock_db)
    assert response["status"] == "success"
