...
async def test_get_carbon_footprint_success(mock_adapter_class):
    user = MagicMock()
    ...
    mock_adapter.get_gross_usage = AsyncMock(return_value=[])
...
@pytest.mark.asyncio
async def test_get_carbon_budget_no_connection():
    user = MagicMock()
    user.tenant_id = "tenant-123"
    db = AsyncMock()
...
async def test_get_carbon_budget_success(mock_budget_service_class, mock_adapter_class):
    user = MagicMock()
    user.tenant_id = "tenant-123"
    db = AsyncMock()
