import uuid
from datetime import date, datetime, timezone
from decimal import Decimal

import pytest


@pytest.mark.asyncio
async def test_quarterly_commercial_report_json_and_csv(
    async_client, app, db, test_tenant
):
    from app.shared.core.auth import CurrentUser, get_current_user, UserRole
    from app.shared.core.pricing import PricingTier
    from app.models.cloud import CloudAccount, CostRecord

    user = CurrentUser(
        id=uuid.uuid4(),
        email="quarterly@valdrix.io",
        tenant_id=test_tenant.id,
        role=UserRole.ADMIN,
        tier=PricingTier.PRO,
    )
    app.dependency_overrides[get_current_user] = lambda: user
    try:
        account = CloudAccount(
            tenant_id=test_tenant.id,
            provider="aws",
            name="Quarterly AWS",
            is_active=True,
        )
        db.add(account)
        await db.flush()
        db.add(
            CostRecord(
                tenant_id=test_tenant.id,
                account_id=account.id,
                service="AmazonEC2",
                region="us-east-1",
                usage_type="BoxUsage",
                cost_usd=Decimal("33.00"),
                currency="USD",
                canonical_charge_category="compute",
                canonical_mapping_version="focus-1.3-v1",
                is_preliminary=False,
                cost_status="FINAL",
                recorded_at=date(2026, 2, 1),
                timestamp=datetime(2026, 2, 1, 10, 0, tzinfo=timezone.utc),
            )
        )
        await db.commit()

        res = await async_client.get(
            "/api/v1/leadership/reports/quarterly",
            params={"year": 2026, "quarter": 1, "response_format": "json"},
        )
        assert res.status_code == 200
        body = res.json()
        assert body["year"] == 2026
        assert body["quarter"] == 1
        assert body["leadership_kpis"]["total_cost_usd"] == 33.0

        csv_res = await async_client.get(
            "/api/v1/leadership/reports/quarterly",
            params={"year": 2026, "quarter": 1, "response_format": "csv"},
        )
        assert csv_res.status_code == 200
        assert csv_res.headers.get("content-type", "").startswith("text/csv")
        assert "year,quarter" in csv_res.text
        assert "2026,1" in csv_res.text
    finally:
        app.dependency_overrides.pop(get_current_user, None)


@pytest.mark.asyncio
async def test_capture_and_list_quarterly_commercial_report_evidence(
    async_client, app, db, test_tenant
):
    from app.shared.core.auth import CurrentUser, get_current_user, UserRole
    from app.shared.core.pricing import PricingTier
    from app.models.cloud import CloudAccount, CostRecord
    from app.models.tenant import User
    from app.modules.governance.domain.security.audit_log import (
        AuditEventType,
        AuditLog,
    )
    from sqlalchemy import select

    admin_user = CurrentUser(
        id=uuid.uuid4(),
        email="admin-quarterly@valdrix.io",
        tenant_id=test_tenant.id,
        role=UserRole.ADMIN,
        tier=PricingTier.PRO,
    )
    db.add(
        User(
            id=admin_user.id,
            tenant_id=test_tenant.id,
            email=admin_user.email,
            role=UserRole.ADMIN,
        )
    )
    await db.commit()

    app.dependency_overrides[get_current_user] = lambda: admin_user
    try:
        account = CloudAccount(
            tenant_id=test_tenant.id,
            provider="aws",
            name="Quarterly AWS",
            is_active=True,
        )
        db.add(account)
        await db.flush()
        db.add(
            CostRecord(
                tenant_id=test_tenant.id,
                account_id=account.id,
                service="AmazonS3",
                region="us-east-1",
                usage_type="Storage",
                cost_usd=Decimal("25.00"),
                currency="USD",
                canonical_charge_category="storage",
                canonical_mapping_version="focus-1.3-v1",
                is_preliminary=False,
                cost_status="FINAL",
                recorded_at=date(2026, 1, 15),
                timestamp=datetime(2026, 1, 15, 10, 0, tzinfo=timezone.utc),
            )
        )
        await db.commit()

        capture = await async_client.post(
            "/api/v1/leadership/reports/quarterly/capture",
            params={"year": 2026, "quarter": 1},
        )
        assert capture.status_code == 200
        body = capture.json()
        assert body["status"] == "captured"
        assert body["report"]["year"] == 2026
        assert body["report"]["quarter"] == 1

        listed = await async_client.get(
            "/api/v1/leadership/reports/quarterly/evidence",
            params={"limit": 10},
        )
        assert listed.status_code == 200
        payload = listed.json()
        assert payload["total"] >= 1
        assert payload["items"][0]["year"] == 2026

        row = await db.scalar(
            select(AuditLog).where(
                AuditLog.tenant_id == test_tenant.id,
                AuditLog.event_type
                == AuditEventType.COMMERCIAL_QUARTERLY_REPORT_CAPTURED.value,
            )
        )
        assert row is not None
    finally:
        app.dependency_overrides.pop(get_current_user, None)
