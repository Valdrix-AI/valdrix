...
class TestOrganizationsDiscoveryDeep:
    ...
    async def test_sync_accounts_success(self, mock_db):
        ...
        mock_sts = MagicMock()
        mock_sts.assume_role = AsyncMock(return_value={"Credentials": {"AccessKeyId": "AKIA", "SecretAccessKey": "S", "SessionToken": "T"}})
        ...
        with patch("aioboto3.Session") as mock_session_class:
            ...
            mock_session.client.side_effect = lambda s, **k: mock_sts if s == "sts" else mock_org
    ...
    async def test_sync_accounts_update_existing(self, mock_db):
        conn = AWSConnection(id=uuid4(), is_management_account=True, aws_account_id="123", role_arn="arn:aws:role", external_id="eid")
        
        mock_sts = MagicMock()
        mock_sts.assume_role = AsyncMock(return_value={"Credentials": {"AccessKeyId": "A", "SecretAccessKey": "S", "SessionToken": "T"}})
        mock_sts.__aenter__ = AsyncMock(return_value=mock_sts)
        mock_sts.__aexit__ = AsyncMock()
