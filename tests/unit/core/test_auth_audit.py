...
from app.shared.core.auth import (
    create_access_token, decode_jwt, get_current_user_from_jwt, get_current_user,
    requires_role, require_tenant_access, CurrentUser
)
...
def test_decode_jwt_valid():
    token = jwt.encode(
        {"sub": "123", "aud": "authenticated", "exp": datetime.now(timezone.utc) + timedelta(hours=1)},
        "supersecretkey",
        algorithm="HS256"
    )
...
async def test_get_current_user_success():
    ...
    mock_result.one_or_none.return_value = (mock_user, PricingTier.PRO)
    mock_db.execute.return_value = mock_result
    
    with patch("app.shared.core.auth.set_session_tenant_id", new_callable=AsyncMock) as mock_rls:
...
async def test_get_current_user_not_found():
    ...
    with patch("app.shared.core.auth.logger") as mock_logger:
        ...
        if exc.value.status_code == 500:
             print(f"DEBUG: 500 Details: {exc.value.detail}")
