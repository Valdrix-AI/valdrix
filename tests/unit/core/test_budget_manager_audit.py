...
async def test_budget_alert_logic():
    ...
    # Patch the config module directly since it's imported inside the function
    with patch("app.shared.core.config.get_settings") as mock_settings:
        mock_settings.return_value.SLACK_BOT_TOKEN = "x"
        mock_settings.return_value.SLACK_CHANNEL_ID = "y"
        
        with patch("app.modules.notifications.domain.SlackService") as mock_slack:
