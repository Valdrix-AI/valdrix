...
async def test_verify_connection_success_aws(service, mock_db):
    ...
    connection = AWSConnection(id=conn_id, tenant_id=tenant_id, aws_account_id="123", status="pending")
...
async def test_verify_connection_failed_azure(service, mock_db):
    ...
    connection = AzureConnection(id=conn_id, tenant_id=tenant_id, subscription_id="sub-1", is_active=False)
...
async def test_verify_connection_internal_error(service, mock_db):
    ...
    with patch("app.shared.adapters.factory.AdapterFactory.get_adapter", side_effect=Exception("API Error")):
        with pytest.raises(HTTPException) as exc:
