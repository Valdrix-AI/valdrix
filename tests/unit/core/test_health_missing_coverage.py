...
class TestHealthServiceMissingCoverage:
    ...
    async def test_check_all_aws_degraded_only(self, health_service, mock_db):
        ...
        with patch.object(health_service, "_check_database", return_value={"status": "up", "latency_ms": 10.5}), \
             patch.object(health_service, "_check_cache", return_value={"status": "healthy", "latency_ms": 5.2}), \
             patch.object(health_service, "_check_external_services", return_value={"status": "degraded", "services": {"aws_sts": {"status": "unhealthy"}}}), \
             patch.object(health_service, "_check_circuit_breakers", return_value={"status": "healthy"}), \
             patch.object(health_service, "_check_system_resources", return_value={"status": "healthy"}), \
             patch.object(health_service, "_check_background_jobs", return_value={"status": "healthy"}):
    ...
    async def test_check_all_redis_degraded_only(self, health_service, mock_db):
        ...
        with patch.object(health_service, "_check_database", return_value={"status": "up", "latency_ms": 10.5}), \
             patch.object(health_service, "_check_cache", return_value={"status": "degraded", "message": "Redis down"}), \
             patch.object(health_service, "_check_external_services", return_value={"status": "healthy", "services": {"aws_sts": {"status": "healthy"}}}), \
             patch.object(health_service, "_check_circuit_breakers", return_value={"status": "healthy"}), \
             patch.object(health_service, "_check_system_resources", return_value={"status": "healthy"}), \
             patch.object(health_service, "_check_background_jobs", return_value={"status": "healthy"}):
            result = await health_service.check_all()
    ...
    async def test_check_all_unhealthy_db_down(self, health_service):
        ...
        with patch.object(health_service, "_check_database", return_value={"status": "down", "error": "Connection failed"}), \
             patch.object(health_service, "_check_cache", return_value={"status": "healthy", "latency_ms": 5.2}), \
             patch.object(health_service, "_check_external_services", return_value={"status": "healthy", "services": {"aws_sts": {"status": "healthy"}}}), \
             patch.object(health_service, "_check_circuit_breakers", return_value={"status": "healthy"}), \
             patch.object(health_service, "_check_system_resources", return_value={"status": "healthy"}), \
    ...
    async def test_check_database_exception(self, health_service, mock_db):
        ...
        with patch("app.shared.core.health.db_health_check", side_effect=Exception("DB error")):
    ...
    async def test_check_database_success(self, health_service, mock_db):
        ...
        with patch("app.shared.core.health.db_health_check", return_value={"status": "up", "latency_ms": 1.0}):
            success, details = await health_service.check_database()
    ...
    async def test_check_all_healthy(self, health_service):
        ...
        with patch.object(health_service, '_check_database', return_value={"status": "up", "latency_ms": 10.5}):
            with patch.object(health_service, '_check_cache', return_value={"status": "healthy", "latency_ms": 5.2}):
                with patch.object(health_service, '_check_external_services', return_value={"status": "healthy", "services": {"aws_sts": {"status": "healthy"}}}):
                    with patch.object(health_service, '_check_circuit_breakers', return_value={"status": "healthy"}):
                        with patch.object(health_service, '_check_system_resources', return_value={"status": "healthy"}):
                            with patch.object(health_service, '_check_background_jobs', return_value={"status": "healthy"}):
                                result = await health_service.check_all()
