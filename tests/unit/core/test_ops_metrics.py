...
def test_zombie_metrics_behavior():
    ...
    ops_metrics.POTENTIAL_SAVINGS.labels(
        provider="aws", 
        account_id="123456789012"
    ).set(99.99)
...
def test_existing_metrics_integrity():
    ...
    ops_metrics.API_ERRORS_TOTAL.labels(path="/test", method="GET", status_code="500").inc()
    val = REGISTRY.get_sample_value(
...
def test_record_circuit_breaker_metrics():
    ...
    with patch("app.shared.core.ops_metrics.CIRCUIT_BREAKER_STATE") as mock_state, \
         patch("app.shared.core.ops_metrics.CIRCUIT_BREAKER_FAILURES") as mock_failures, \
         patch("app.shared.core.ops_metrics.CIRCUIT_BREAKER_RECOVERIES") as mock_recoveries:
...
def test_record_retry_and_timeout_metrics():
    with patch("app.shared.core.ops_metrics.OPERATION_RETRIES_TOTAL") as mock_retries, \
