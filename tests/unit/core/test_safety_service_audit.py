...
async def test_monthly_hard_cap_triggered(service, mock_db):
    ...
    with patch("app.modules.reporting.domain.aggregator.CostAggregator.get_summary", return_value=mock_summary):
        with patch("app.shared.core.notifications.NotificationDispatcher.notify_budget_alert", new_callable=AsyncMock) as mock_notify:
            with pytest.raises(KillSwitchTriggeredError, match="Monthly budget hard-cap reached"):
...
async def test_circuit_breaker_triggered(service, mock_db):
    ...
    with pytest.raises(KillSwitchTriggeredError, match="Remediation circuit breaker open"):
