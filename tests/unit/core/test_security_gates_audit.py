...
def member_user():
    return CurrentUser(
        id=uuid4(), email="mem@ex.com", role=UserRole.MEMBER, tier=PricingTier.STARTER
    )
...
def admin_user():
    return CurrentUser(
        id=uuid4(), email="adm@ex.com", role=UserRole.ADMIN, tier=PricingTier.PRO
    )
...
def owner_user():
    return CurrentUser(
        id=uuid4(), email="own@ex.com", role=UserRole.OWNER, tier=PricingTier.ENTERPRISE
    )
