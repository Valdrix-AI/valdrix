...
async def test_oidc_discovery(async_client: AsyncClient):
    ...
    with patch("app.shared.connections.oidc.OIDCService.get_discovery_doc", new_callable=AsyncMock) as mock_get:
...
async def test_jwks_endpoint(async_client: AsyncClient):
    ...
    with patch("app.shared.connections.oidc.OIDCService.get_jwks", new_callable=AsyncMock) as mock_get:
