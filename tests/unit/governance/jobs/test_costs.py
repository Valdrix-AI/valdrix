...
async def test_cost_ingestion_with_connection(mock_job, mock_db):
    ...
    with (
        ...
        with patch.dict(
            "sys.modules",
            {"app.modules.reporting.domain.attribution_engine": MagicMock()},
        ):
            with patch(
...
async def test_cost_forecast_handler(mock_job, mock_db):
    ...
    mock_job.payload = {"start_date": "2023-01-01", "end_date": "2023-01-31"}
...
async def test_cost_ingestion_honors_backfill_window(mock_db):
    ...
    assert stream_kwargs["start_date"] == datetime(
        2026, 1, 1, 0, 0, tzinfo=timezone.utc
    )
    assert stream_kwargs["end_date"] == datetime(
        2026, 1, 10, 23, 59, 59, 999999, tzinfo=timezone.utc
    )
