...
async def test_onboard_endpoint(async_client: AsyncClient, app, mock_user):
    ...
    response = await async_client.post("/api/v1/settings/onboard", json={"tenant_name": "TestTenant"})
    assert response.status_code == 200
...
async def test_onboard_duplicate(async_client: AsyncClient, app, mock_user):
    ...
    # First onboarding
    await async_client.post("/api/v1/settings/onboard", json={"tenant_name": "DuplicateTenant"})
    
    # Second onboarding
    response = await async_client.post("/api/v1/settings/onboard", json={"tenant_name": "DuplicateTenant"})
    assert response.status_code == 400
    data = response.json()
...
@pytest.mark.asyncio
async def test_onboard_with_cloud_config_multi(async_client: AsyncClient, app, mock_user):
    app.dependency_overrides[get_current_user_from_jwt] = lambda: mock_user
    ...
    for platform, config in platforms:
        ...
        with patch("app.shared.adapters.factory.AdapterFactory.get_adapter", return_value=mock_adapter):
            ...
            fresh_user = CurrentUser(id=uuid.uuid4(), tenant_id=None, email=f"{platform}@test.ai", role=UserRole.MEMBER)
