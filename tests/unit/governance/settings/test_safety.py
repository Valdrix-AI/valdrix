...
@pytest.mark.asyncio
async def test_get_safety_status(async_client: AsyncClient, mock_user_id, mock_tenant_id, app):
    ...
    with patch("app.shared.remediation.circuit_breaker.get_circuit_breaker", return_value=mock_cb):
        try:
...
@pytest.mark.asyncio
async def test_reset_circuit_breaker(async_client: AsyncClient, mock_user_id, mock_tenant_id, app):
...
@pytest.mark.asyncio
async def test_get_safety_status_exception_returns_defaults(async_client: AsyncClient, mock_user_id, mock_tenant_id, app):
    user_id = uuid.UUID(str(mock_user_id))
    tenant_id = uuid.UUID(str(mock_tenant_id))
    mock_user = CurrentUser(id=user_id, tenant_id=tenant_id, email="test@valdrix.io", role=UserRole.ADMIN)

    app.dependency_overrides[get_current_user] = lambda: mock_user
    with patch("app.shared.remediation.circuit_breaker.get_circuit_breaker", side_effect=RuntimeError("redis down")), \
...
@pytest.mark.asyncio
async def test_reset_circuit_breaker_failure(async_client: AsyncClient, mock_user_id, mock_tenant_id, app):
    user_id = uuid.UUID(str(mock_user_id))
    tenant_id = uuid.UUID(str(mock_tenant_id))
    mock_user = CurrentUser(id=user_id, tenant_id=tenant_id, email="test@valdrix.io", role=UserRole.ADMIN)
    ...
    with patch("app.shared.remediation.circuit_breaker.get_circuit_breaker", return_value=mock_cb):
        try:
            ...
            body = response.json()
            assert "Failed to reset circuit breaker" in (body.get("message") or body.get("error") or "")
        finally:
            app.dependency_overrides.pop(get_current_user, None)


@pytest.mark.asyncio
async def test_reset_circuit_breaker_requires_admin(async_client: AsyncClient, mock_user_id, mock_tenant_id, app):
    user_id = uuid.UUID(str(mock_user_id))
    tenant_id = uuid.UUID(str(mock_tenant_id))
    mock_user = CurrentUser(id=user_id, tenant_id=tenant_id, email="member@valdrix.io", role=UserRole.MEMBER)
