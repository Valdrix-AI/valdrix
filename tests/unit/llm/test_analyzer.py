    ...
    def test_load_system_prompt_from_yaml(self, mock_llm):
        ...
        with patch('os.path.exists', return_value=True), \
            ...
            mock_yaml.return_value = {
                "finops_analysis": {
                    "system": "Test system prompt from YAML"
                }
            }
...
class TestFinOpsAnalyzerAnalysis:
    ...
    @pytest.mark.asyncio
    async def test_analyze_budget_exceeded_error(self, analyzer, shared_usage_summary, mock_db):
        """Test analysis fails when budget is exceeded."""
        tenant_id = uuid4()

        with patch.object(analyzer, '_check_cache_and_delta', return_value=(None, False)), \
             patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', new_callable=AsyncMock) as mock_budget:
            mock_budget.side_effect = BudgetExceededError("Budget exceeded")

            with pytest.raises(BudgetExceededError):
                await analyzer.analyze(shared_usage_summary, tenant_id=tenant_id, db=mock_db)

    @pytest.mark.asyncio
    async def test_analyze_successful_flow(self, analyzer, shared_usage_summary, mock_db):
        ...
        with patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', new_callable=AsyncMock) as mock_reserve, \
             ...
             patch('app.shared.llm.analyzer.LLMGuardrails.sanitize_input') as mock_sanitize, \
            ...
            mock_invoke.return_value = ('{"summary": "test"}', {"token_usage": {"prompt_tokens": 500, "completion_tokens": 500}})
            mock_process.return_value = {"result": "success"}

            result = await analyzer.analyze(shared_usage_summary, tenant_id=tenant_id, db=mock_db)
    ...
    async def test_analyze_cache_hit(self, analyzer, shared_usage_summary):
        ...
        with patch.object(analyzer, '_check_cache_and_delta', return_value=(cached_result, False)):
            result = await analyzer.analyze(shared_usage_summary, tenant_id=tenant_id)

            assert result == cached_result

    @pytest.mark.asyncio
    async def test_analyze_data_preparation_failure(self, analyzer, shared_usage_summary, mock_db):
        ...
        with patch.object(analyzer, '_check_cache_and_delta', return_value=(None, False)), \
             ...
             patch('app.shared.llm.analyzer.LLMGuardrails.sanitize_input', side_effect=Exception("Data error")):
    ...
    @pytest.mark.asyncio
    async def test_analyze_llm_invocation_failure(self, analyzer, shared_usage_summary, mock_db):
        """Test analysis handles LLM invocation failures."""
        tenant_id = uuid4()

        with patch.object(analyzer, '_check_cache_and_delta', return_value=(None, False)), \
             patch('app.shared.llm.analyzer.LLMGuardrails.sanitize_input', return_value={"test": "data"}), \
             patch('app.shared.llm.analyzer.SymbolicForecaster.forecast', return_value={"forecast": "test"}), \
             patch('app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve', new_callable=AsyncMock) as mock_reserve, \
...
class TestFinOpsAnalyzerCaching:
    ...
    @pytest.mark.asyncio
    async def test_check_cache_and_delta_no_tenant(self, analyzer, shared_usage_summary):
        """Test cache check returns None when no tenant ID."""
        result = await analyzer._check_cache_and_delta(None, False, shared_usage_summary)

        assert result == (None, False)

    @pytest.mark.asyncio
    async def test_check_cache_and_delta_cache_hit(self, analyzer, shared_usage_summary):
        ...
        with patch('app.shared.llm.analyzer.get_cache_service') as mock_cache_service, \
            ...
            result = await analyzer._check_cache_and_delta(tenant_id, False, shared_usage_summary)

            assert result == (cached_data, False)

    @pytest.mark.asyncio
    async def test_check_cache_and_delta_force_refresh(self, analyzer, shared_usage_summary):
        ...
        with patch('app.shared.llm.analyzer.get_cache_service') as mock_cache_service:
            ...
            result = await analyzer._check_cache_and_delta(tenant_id, True, shared_usage_summary)
...
class TestFinOpsAnalyzerClientSetup:
    ...
    async def test_setup_client_and_usage_basic(self, analyzer, mock_db):
        ...
        with patch('app.shared.llm.analyzer.get_settings') as mock_settings:
            ...
            with patch('app.shared.llm.analyzer.UsageTracker') as mock_usage_tracker:
                 ...
                 result = await analyzer._setup_client_and_usage(tenant_id, mock_db, None, None)
    ...
    async def test_setup_client_and_usage_with_budget(self, analyzer, mock_db):
        ...
        with patch('app.shared.llm.analyzer.UsageTracker') as mock_usage_tracker, \
            ...
            mock_tracker_instance.check_budget = AsyncMock(return_value=MagicMock())  # Not HARD_LIMIT
            mock_usage_tracker.return_value = mock_tracker_instance
    ...
    async def test_setup_client_and_usage_budget_hard_limit(self, analyzer, mock_db):
        ...
        with patch('app.shared.llm.analyzer.UsageTracker') as mock_usage_tracker:
            ...
            mock_tracker_instance.check_budget = AsyncMock(return_value=BudgetStatus.HARD_LIMIT)
            mock_usage_tracker.return_value = mock_tracker_instance
    ...
    @pytest.mark.asyncio
    async def test_setup_client_and_usage_soft_limit_degradation(self, analyzer, mock_db):
        ...
        with patch('app.shared.llm.analyzer.UsageTracker') as mock_usage_tracker, \
            ...
            mock_tracker_instance = MagicMock()
            mock_tracker_instance.check_budget = AsyncMock(return_value=BudgetStatus.SOFT_LIMIT)
            mock_usage_tracker.return_value = mock_tracker_instance
...
class TestFinOpsAnalyzerLLMInvocation:
    ...
    async def test_invoke_llm_success(self, analyzer):
        ...
        with patch('app.shared.llm.analyzer.LLMFactory.create') as mock_factory, \
            ...
            result = await analyzer._invoke_llm("test data", "groq", "llama-3.3-70b-versatile", None)

            content, metadata = result
            assert content == '{"result": "success"}'
            assert metadata == {"token_usage": {"prompt_tokens": 100, "completion_tokens": 50}}
    ...
    async def test_invoke_llm_fallback_on_failure(self, analyzer):
        ...
        with patch('app.shared.llm.analyzer.LLMFactory.create') as mock_factory, \
            ...
            assert metadata == {"token_usage": {"prompt_tokens": 200, "completion_tokens": 100}}
    ...
    async def test_invoke_llm_all_failures(self, analyzer):
        ...
        with patch('app.shared.llm.analyzer.LLMFactory.create') as mock_factory, \
            ...
            with pytest.raises(AIAnalysisError, match="All LLM providers failed"):
                await analyzer._invoke_llm("test data", "groq", "llama-3.3-70b-versatile", None)
...
class TestFinOpsAnalyzerResultProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_analysis_results_success(self, analyzer, shared_usage_summary):
        ...
        with patch('app.shared.llm.analyzer.LLMGuardrails.validate_output') as mock_validate, \
            ...
            result = await analyzer._process_analysis_results('{"test": "content"}', tenant_id, shared_usage_summary)

            expected_keys = ["insights", "recommendations", "anomalies", "forecast", "symbolic_forecast", "llm_raw"]
            assert all(key in result for key in expected_keys)
            mock_cache.set_analysis.assert_called_once_with(tenant_id, result)

    @pytest.mark.asyncio
    async def test_process_analysis_results_validation_failure(self, analyzer, shared_usage_summary):
        """Test result processing with validation failure fallback."""
        tenant_id = uuid4()

        with patch('app.shared.llm.analyzer.LLMGuardrails.validate_output', side_effect=Exception("Validation failed")), \
             patch('app.shared.llm.analyzer.SymbolicForecaster.forecast', new_callable=AsyncMock) as mock_forecast, \
             patch('app.shared.llm.analyzer.get_cache_service') as mock_cache_service:
            ...
            result = await analyzer._process_analysis_results(content, tenant_id, shared_usage_summary)

            assert result["insights"] == ["fallback insight"]
            assert result["recommendations"] == ["fallback rec"]

    @pytest.mark.asyncio
    async def test_process_analysis_results_json_parse_failure(self, analyzer, shared_usage_summary):
    ...
    async def test_check_and_alert_anomalies_slack_enabled(self, analyzer):
        ...
        with patch('app.shared.llm.analyzer.get_settings') as mock_settings, \
             patch('app.shared.llm.analyzer.SlackService') as mock_slack_service:

            mock_settings_obj = MagicMock()
            mock_settings_obj.SLACK_BOT_TOKEN = "xoxb-test"
            mock_settings_obj.SLACK_CHANNEL_ID = "C123456"
            mock_settings.return_value = mock_settings_obj
            ...
            mock_slack_service.assert_called_once_with("xoxb-test", "C123456")
    ...
    async def test_check_and_alert_anomalies_slack_disabled(self, analyzer):
        ...
        with patch('app.shared.llm.analyzer.get_settings') as mock_settings:
            mock_settings_obj = MagicMock()
            mock_settings_obj.SLACK_BOT_TOKEN = None
            mock_settings.return_value = mock_settings_obj

            anomalies = [{"resource": "test", "issue": "test", "cost_impact": "$100", "severity": "medium"}]
            result = {"anomalies": anomalies}
...
class TestFinOpsAnalyzerProductionQuality:
    ...
    async def test_concurrent_analysis_operations(self):
        ...
        usage_summary = CloudUsageSummary(
            ...
            records=[CostRecord(date=date(2024, 1, 1), amount=Decimal("100.0"), service="EC2", region="us-east-1")],
    ...
    async def test_budget_integration_edge_cases(self):
        ...
        with patch.object(analyzer, "_check_cache_and_delta", return_value=(None, False)), \
             patch.object(analyzer, "_setup_client_and_usage", return_value=(None, "groq", "llama-3.3-70b-versatile", None)), \
             patch.object(analyzer, "_invoke_llm", return_value=('{"summary":"ok"}', {"token_usage": {}})), \
    ...
    async def test_caching_integration_comprehensive(self):
        ...
        with patch('app.shared.llm.analyzer.get_cache_service') as mock_cache_service:
            ...
            result = await analyzer._check_cache_and_delta(uuid4(), False, usage_summary)
            assert result == (None, False)
    ...
    async def test_error_propagation_and_logging(self):
        ...
        # Test that exceptions are properly logged and propagated
        with patch.object(analyzer, "_check_cache_and_delta", return_value=(None, False)), \
             patch('app.shared.llm.analyzer.LLMGuardrails.sanitize_input', side_effect=Exception("Sanitization error")), \
