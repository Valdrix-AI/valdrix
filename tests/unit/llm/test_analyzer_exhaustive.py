...
@pytest.mark.asyncio
async def test_llm_invocation_primary_failure_fallback(mock_llm_factory, usage_summary, mock_db):
    ...
    with patch("app.shared.llm.analyzer.get_cache_service", return_value=mock_cache), \
         ...
         patch("app.shared.llm.analyzer.get_settings") as mock_settings, \
         patch("app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve", new_callable=AsyncMock) as mock_reserve, \
         patch("app.shared.llm.analyzer.LLMBudgetManager.record_usage", new_callable=AsyncMock):
...
async def test_process_results_fallback(mock_llm, mock_db, mock_forecaster):
    ...
    with patch("app.shared.llm.analyzer.SymbolicForecaster.forecast", new_callable=AsyncMock) as mock_forecast:
        mock_forecast.return_value = {"forecast": "data"}
        ...
        with patch("app.shared.llm.analyzer.get_cache_service", return_value=mock_cache), \
            ...
            mock_settings.return_value.SLACK_BOT_TOKEN = None
            res = await analyzer._process_analysis_results('{"foo": "bar"}', None, MagicMock(records=[], tenant_id=None))
...
async def test_analyze_force_refresh(mock_llm, usage_summary, mock_db):
    ...
    with patch("app.shared.llm.analyzer.get_cache_service", return_value=mock_cache), \
         ...
         patch.object(analyzer, "_process_analysis_results", new_callable=AsyncMock) as mock_proc:
            ...
            result = await analyzer.analyze(usage_summary, tenant_id=tenant_id, force_refresh=True, db=mock_db)
            assert result == {"fresh": True}
            assert mock_invoke.called

@pytest.mark.asyncio
async def test_analyze_with_delta_analysis_enabled(mock_llm, usage_summary_with_records, mock_db):
    analyzer = FinOpsAnalyzer(mock_llm, mock_db)
    tenant_id = uuid4()
    
    mock_cache = MagicMock()
    # Initial cached content needs records for delta cutoff logic
    mock_cache.get_analysis = AsyncMock(return_value={
        "cached": True, 
    ...
    with patch("app.shared.llm.analyzer.get_cache_service", return_value=mock_cache), \
         ...
         patch.object(analyzer, "_process_analysis_results", new_callable=AsyncMock) as mock_proc:
            ...
            result = await analyzer.analyze(usage_summary_with_records, tenant_id=tenant_id, db=mock_db)
            assert result == {"fresh": True}
            assert mock_invoke.called
...
async def test_invoke_llm_failure_retry(mock_llm_factory, usage_summary, mock_db):
    ...
    with patch("app.shared.llm.analyzer.stop_after_attempt", return_value=stop_after_attempt(1)), \
...
async def test_process_results_with_slack_alert(mock_llm, mock_db):
    ...
    with patch("app.shared.llm.analyzer.get_settings") as mock_settings, \
         ...
         patch("app.shared.llm.analyzer.SymbolicForecaster.forecast", new_callable=AsyncMock) as mock_forecast:
            ...
            await analyzer._process_analysis_results(content, None, MagicMock(records=[], tenant_id=uuid4()))
...
async def test_check_cache_and_delta_no_new_data(mock_llm, usage_summary):
    ...
    with patch("app.shared.llm.analyzer.get_cache_service", return_value=mock_cache), \
         patch("app.shared.llm.analyzer.get_settings") as mock_settings:
            mock_settings.return_value.ENABLE_DELTA_ANALYSIS = True
            mock_settings.return_value.DELTA_ANALYSIS_DAYS = 7
            res, is_delta = await analyzer._check_cache_and_delta(uuid4(), False, usage_summary)
            assert not is_delta

@pytest.mark.asyncio
async def test_analyze_data_prep_failure(mock_llm, usage_summary, mock_db):
    analyzer = FinOpsAnalyzer(mock_llm, mock_db)
    with patch("app.shared.llm.analyzer.LLMGuardrails.sanitize_input", side_effect=Exception("Sanitize fail")), \
         patch("app.shared.llm.analyzer.LLMBudgetManager.check_and_reserve", new_callable=AsyncMock) as mock_reserve, \
...
async def test_process_results_json_failure(mock_llm, mock_db):
    ...
    with patch("app.shared.llm.analyzer.SymbolicForecaster.forecast", return_value={}):
        res = await analyzer._process_analysis_results("Invalid JSON", None, MagicMock(records=[], tenant_id=None))
