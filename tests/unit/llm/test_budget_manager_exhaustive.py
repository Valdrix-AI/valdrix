...
async def test_record_usage(mock_db, tenant_id):
    with patch("app.shared.llm.budget_manager.get_tenant_tier", return_value=MagicMock(value="free")), \
         patch.object(LLMBudgetManager, "_check_budget_and_alert", new_callable=AsyncMock) as mock_alert:
...
async def test_check_budget(mock_db, tenant_id):
    budget = LLMBudget(tenant_id=tenant_id, monthly_limit_usd=10.0, alert_threshold_percent=80)
    res = MagicMock()
    res.scalar_one_or_none.return_value = budget
    res_usage = MagicMock()
