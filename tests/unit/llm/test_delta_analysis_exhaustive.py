...
class TestDeltaAnalysisExhaustive:
    ...
    def test_delta_result_has_significant_changes_variants(self):
        ...
        res2 = DeltaAnalysisResult(tenant_id, today, total_change_percent=SIGNIFICANT_CHANGE_PERCENT + 1)
    ...
    async def test_compute_delta_new_resource_threshold(self, delta_service):
        ...
        curr = [
            {"Groups": [
                {"Keys": ["S1", "tiny"], "Metrics": {"UnblendedCost": {"Amount": "0.10"}}},
                {"Keys": ["S1", "big"], "Metrics": {"UnblendedCost": {"Amount": "1.10"}}}
            ]}
        ]
        
        res = await delta_service.compute_delta(uuid4(), curr, previous_costs=None, days_to_compare=1)
    ...
    async def test_compute_delta_removed_resource(self, delta_service):
        ...
        prev = [{"Groups": [{"Keys": ["S1", "gone"], "Metrics": {"UnblendedCost": {"Amount": "10.0"}}}]}]
        curr = []
        
        res = await delta_service.compute_delta(uuid4(), curr, previous_costs=prev, days_to_compare=1)
    ...
    async def test_compute_delta_insignificant_change(self, delta_service):
        ...
        curr = [{"Groups": [{"Keys": ["S1", "r"], "Metrics": {"UnblendedCost": {"Amount": "10.25"}}}]}] # $0.25 change
    ...
    def test_aggregate_malformed_aws_data(self, delta_service):
        ...
        bad_data = [
            {"Groups": [
                ...
                {"Keys": [], "Metrics": {"UnblendedCost": {"Amount": "1.0"}}} # No Keys
            ]}
        ]
