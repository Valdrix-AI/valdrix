...
def test_flatten_zombies(analyzer):
    ...
    raw_data = {
        ...
        "ebs_unattached": [{"resource_id": "vol-789", "cost": 10.0}],
    }
...
async def test_analyze_flow(analyzer, mock_llm):
    ...
    with patch("app.shared.llm.zombie_analyzer.get_settings") as mock_settings_fn:
        ...
        with patch(
            "app.shared.llm.zombie_analyzer.ChatPromptTemplate"
        ) as mock_prompt_cls:
            ...
            fresh_analyzer._get_effective_llm_config = AsyncMock(
                return_value=("openai", "gpt-4", None)
            )
