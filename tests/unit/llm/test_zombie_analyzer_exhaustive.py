...
async def test_zombie_analyzer_byok_resolution(zombie_analyzer):
    ...
    with patch("app.shared.llm.zombie_analyzer.get_settings") as mock_settings, \
         ...
         patch("app.shared.llm.zombie_analyzer.LLMGuardrails") as mock_guardrails: # Patch Guardrails
        
        mock_settings.return_value.ZOMBIE_PLUGIN_TIMEOUT_SECONDS = 30
        mock_guardrails.sanitize_input = AsyncMock(return_value=[])
        mock_guardrails.validate_output.return_value = MagicMock(model_dump=lambda: {"resources": []})
        ...
        await zombie_analyzer.analyze({"ec2": [{"id": "i-1"}]}, tenant_id=tenant_id, db=mock_db)
...
async def test_zombie_analyzer_claude_byok(zombie_analyzer):
    ...
    mock_chain = AsyncMock()
    mock_chain.ainvoke.return_value = MagicMock(content='{"summary": "test", "total_monthly_savings": "$0", "resources": []}')
    zombie_analyzer.prompt = MagicMock()
    zombie_analyzer.prompt.__or__.return_value = mock_chain
...
async def test_zombie_analyzer_malformed_response(zombie_analyzer):
    ...
    with patch("app.shared.llm.factory.LLMFactory", new_callable=MagicMock), \
         patch("app.shared.llm.zombie_analyzer.LLMGuardrails") as mock_guardrails: # Patch guardrails to raise error
        
        mock_guardrails.sanitize_input = AsyncMock(return_value=[])
        # validate_output raises ValueError on bad JSON usually
        mock_guardrails.validate_output.side_effect = ValueError("Invalid JSON")

        results = await zombie_analyzer.analyze({"ec2": [{"id": "i-1"}]}, tenant_id=uuid4())
...
async def test_usage_tracking_exception(zombie_analyzer):
    ...
    zombie_analyzer = ZombieAnalyzer(mock_llm_client) # Re-init to use real methods if needed
    ...
    with patch("app.shared.llm.zombie_analyzer.UsageTracker") as MockTracker, \
