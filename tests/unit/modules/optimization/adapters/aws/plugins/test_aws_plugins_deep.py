...
async def test_unused_elastic_ips_detects_unattached(monkeypatch):
    ...
    ec2 = MagicMock()
    ec2.describe_addresses = AsyncMock(return_value={
        "Addresses": [
    ...
    with patch("app.modules.reporting.domain.pricing.service.PricingService.estimate_monthly_waste", return_value=3.6):
...
async def test_idle_instances_cloudwatch_detection(monkeypatch):
    ...
    with patch("app.modules.optimization.adapters.aws.plugins.compute.cloudwatch_limiter.acquire", new=AsyncMock()), \
...
async def test_orphan_load_balancers_no_healthy_targets(monkeypatch):
    ...
    elb.get_paginator.side_effect = [
        AsyncPaginator([{"LoadBalancers": [{"LoadBalancerArn": "arn-1", "LoadBalancerName": "lb1", "Type": "application"}]}]),
        AsyncPaginator([{"TargetGroups": [{"TargetGroupArn": "tg-1"}]}]),
    ]
    elb.describe_target_health = AsyncMock(return_value={"TargetHealthDescriptions": [{"TargetHealth": {"State": "unhealthy"}}]})

    monkeypatch.setattr(plugin, "_get_client", lambda *args, **kwargs: AsyncContext(elb))

    with patch("app.modules.reporting.domain.pricing.service.PricingService.estimate_monthly_waste", return_value=20.0):
        zombies = await plugin.scan(session=MagicMock(), region="us-east-1")
        assert len(zombies) == 1
...
async def test_underused_nat_gateways_cloudwatch(monkeypatch):
    ...
    with patch("app.modules.reporting.domain.pricing.service.PricingService.estimate_monthly_waste", return_value=32.4):
        zombies = await plugin.scan(session=MagicMock(), region="us-east-1")
        assert len(zombies) == 1
        assert zombies[0]["resource_id"] == "nat-1"
...
async def test_unattached_volumes_detected(monkeypatch):
    ...
    ec2 = MagicMock()
    ec2.get_paginator.return_value = AsyncPaginator([
        {"Volumes": [{"VolumeId": "vol-1", "Size": 10, "CreateTime": datetime.now(timezone.utc) - timedelta(days=20)}]}
    ])
    ...
    with patch("app.modules.reporting.domain.pricing.service.PricingService.estimate_monthly_waste", return_value=5.0):
        zombies = await plugin.scan(session=MagicMock(), region="us-east-1")
        assert len(zombies) == 1
...
async def test_old_snapshots_detected(monkeypatch):
    ...
    ec2 = MagicMock()
    ec2.get_paginator.return_value = AsyncPaginator([
        {"Snapshots": [{"SnapshotId": "snap-1", "StartTime": old_time, "VolumeSize": 50}]}
    ])

    monkeypatch.setattr(plugin, "_get_client", lambda *args, **kwargs: AsyncContext(ec2))

    with patch("app.modules.reporting.domain.pricing.service.PricingService.estimate_monthly_waste", return_value=2.5):
        zombies = await plugin.scan(session=MagicMock(), region="us-east-1")
        assert len(zombies) == 1
