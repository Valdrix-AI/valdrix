...
async def test_stopped_instances_with_ebs_scan(mock_session):
    ...
    mock_ec2.describe_volumes = AsyncMock(
        return_value={
            "Volumes": [{"VolumeId": "vol-123", "Size": 100, "VolumeType": "gp3"}]
        }
    )
...
async def test_stopped_instances_unparseable_reason(mock_session):
    ...
    mock_ec2.describe_volumes = AsyncMock(
        return_value={
            "Volumes": [{"VolumeId": "vol-456", "Size": 10, "VolumeType": "gp2"}]
        }
    )
    mock_session.client.return_value.__aenter__.return_value = mock_ec2
...
async def test_unused_lambda_scan_none(mock_session):
    ...
    mock_paginator = MagicMock()
    mock_paginator.paginate.return_value.__aiter__.return_value = [
        {"Functions": [{"FunctionName": "active-func"}]}
    ]
    ...
    mock_cw.get_metric_statistics = AsyncMock(
        return_value={"Datapoints": [{"Sum": 5.0}]}
    )
    ...
    mock_session.client.side_effect = lambda s, **kw: MagicMock(
        __aenter__=AsyncMock(return_value=side_effect(s))
    )
    results = await plugin.scan(mock_session, "us-east-1")
...
async def test_orphan_vpc_endpoints_scan_none(mock_session):
    ...
    # Mock traffic > 0
    mock_cw.get_metric_statistics = AsyncMock(
        return_value={"Datapoints": [{"Sum": 100.0}]}
    )
...
async def test_lambda_scan_full_error(mock_session):
    ...
    mock_session.client.side_effect = ClientError(
        {"Error": {"Code": "500", "Message": "Fail"}}, "client"
    )
    results = await plugin.scan(mock_session, "us-east-1")
    assert len(results) == 0
...
async def test_stopped_instances_malformed_date(mock_session):
    ...
    mock_ec2.describe_volumes = AsyncMock(
        return_value={
            "Volumes": [{"VolumeId": "vol-789", "Size": 10, "VolumeType": "gp2"}]
        }
    )
    mock_session.client.return_value.__aenter__.return_value = mock_ec2
    results = await plugin.scan(mock_session, "us-east-1")
...
async def test_stopped_instances_client_error(mock_session):
    ...
    mock_ec2.describe_volumes = AsyncMock(
        side_effect=ClientError(
            {"Error": {"Code": "AccessDenied", "Message": "Denied"}}, "describe_volumes"
        )
    )
    mock_session.client.return_value.__aenter__.return_value = mock_ec2
