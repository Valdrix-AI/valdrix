...
async def test_slack_rate_limit_retry():
    ...
    rate_limit_err.response.get.side_effect = lambda k, d=None: "ratelimited" if k == 'error' else d
...
async def test_email_carbon_alert():
    ...
    service = EmailService("smtp.example.com", 587, "user", "pass", "no-reply@example.com")
