...
async def test_hot_region_invalid_days_falls_back_to_enabled():
    rd = RegionDiscovery()
    with patch.object(
        rd, "get_enabled_regions", AsyncMock(return_value=["us-east-1"])
    ) as mock_enabled:
...
async def test_hot_region_empty_dimension_values_fallback():
    ...
    with patch(
        ...
        rd = RegionDiscovery(credentials={"AccessKeyId": "ak", "SecretAccessKey": "sk"})
        with patch.object(
            rd, "get_enabled_regions", AsyncMock(return_value=["us-west-2"])
        ) as mock_enabled:
...
async def test_enabled_regions_uses_cache_without_session():
    ...
    rd.session.client.side_effect = AssertionError(
        "should not call session when cached"
    )
