...
from app.modules.optimization.domain.remediation import RemediationAction, RemediationService
...
async def test_backup_creation_errors_raise(service: RemediationService) -> None:
    ...
    async def fake_client(name: str) -> _AsyncContextManager:
        return _AsyncContextManager({"ec2": ec2, "rds": rds, "redshift": redshift}[name])
...
@pytest.mark.asyncio
async def test_execute_action_delete_volume_detach_and_delete(service: RemediationService) -> None:
    ec2 = AsyncMock()
...
@pytest.mark.asyncio
async def test_execute_action_delete_volume_detach_timeout(service: RemediationService) -> None:
    ec2 = AsyncMock()
    ec2.describe_volumes.side_effect = [
    ...
    with patch("app.modules.optimization.domain.remediation.time.monotonic", side_effect=[0, 0, 181]), patch(
        "app.modules.optimization.domain.remediation.asyncio.sleep",
        new=AsyncMock(),
    ):
        with pytest.raises(ValueError, match="did not detach"):
            await service._execute_action("vol-timeout", RemediationAction.DELETE_VOLUME)


@pytest.mark.asyncio
async def test_enforce_hard_limit_non_hard_limit_short_circuit(service: RemediationService) -> None:
    tenant_id = uuid4()
    with patch("app.shared.llm.usage_tracker.UsageTracker") as usage_tracker_cls:
        usage_tracker_cls.return_value.check_budget = AsyncMock(return_value=BudgetStatus.SOFT_LIMIT)
        result = await service.enforce_hard_limit(tenant_id)
    assert result == []


@pytest.mark.asyncio
async def test_enforce_hard_limit_execute_errors_are_captured(service: RemediationService) -> None:
    tenant_id = uuid4()
    req = MagicMock()
    req.id = uuid4()
    ...
    with patch("app.shared.llm.usage_tracker.UsageTracker") as usage_tracker_cls, patch(
        "app.modules.optimization.domain.remediation.get_settings"
    ) as get_settings:
        usage_tracker_cls.return_value.check_budget = AsyncMock(return_value=BudgetStatus.HARD_LIMIT)
...
@pytest.mark.asyncio
async def test_generate_iac_plan_provider_paths_and_bulk(service: RemediationService) -> None:
    req = MagicMock()
    ...
    with patch("app.shared.core.pricing.get_tenant_tier", new=AsyncMock(return_value="pro")), patch(
        "app.shared.core.pricing.is_feature_enabled", return_value=True
    ):
