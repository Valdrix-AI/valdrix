...
from app.modules.optimization.domain.remediation import RemediationService, RemediationStatus, RemediationAction
...
async def test_execute_approved_triggers_grace_period(mock_db, tenant_id):
    ...
    with patch("app.modules.optimization.domain.remediation.SafetyGuardrailService", return_value=mock_safety):
        # Mock audit logger
        with patch("app.modules.optimization.domain.remediation.AuditLogger") as mock_audit:
...
async def test_execute_immediate_action_success(mock_db, tenant_id):
    ...
    with patch("app.modules.optimization.domain.remediation.SafetyGuardrailService") as MockSafety, \
         ...
         patch("app.shared.core.notifications.NotificationDispatcher") as MockNotify: # Patch Source for this one as it is imported inside method
...
async def test_generate_iac_plan_pro_tier(mock_db, tenant_id):
    ...
    with patch("app.shared.core.pricing.get_tenant_tier", AsyncMock(return_value="starter")):
        with patch("app.shared.core.pricing.is_feature_enabled", return_value=True):
            ...
            assert "resource \"aws_instance\"" in plan or "terraform state rm aws_instance" in plan
