...
async def test_update_rule_normalizes_type(mock_db, engine, tenant_id):
    ...
    with patch.object(engine, "get_rule", new=AsyncMock(return_value=rule)):
        updated = await engine.update_rule(
            tenant_id,
            rule.id,
            {"rule_type": "percentage", "name": "Updated", "allocation": {"bucket": "Ops"}},
        )
...
async def test_apply_rules_percentage_mismatch_logs_warning(engine):
    ...
    with patch.object(engine, "match_conditions", return_value=True), \
         patch("app.modules.reporting.domain.attribution_engine.logger.warning") as mock_warning:
...
@pytest.mark.asyncio
async def test_apply_rules_to_tenant_no_records_logs_and_returns_zero(mock_db, engine, tenant_id):
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = []
    mock_db.execute.return_value = mock_result
    with patch("app.modules.reporting.domain.attribution_engine.logger.info") as mock_info:
        result = await engine.apply_rules_to_tenant(tenant_id, date(2026, 1, 1), date(2026, 1, 31))
