...
@pytest.mark.asyncio
async def test_save_summary_postgresql_path(
    persistence_service, mock_db, sample_summary
):
...
@pytest.mark.asyncio
async def test_save_summary_sqlite_idempotency(
    persistence_service, mock_db, sample_summary
):
    mock_db.bind.url = "sqlite+aiosqlite:///:memory:"
    account_id = str(uuid4())
...
async def test_save_records_stream(persistence_service, mock_db):
    ...
    result = await persistence_service.save_records_stream(
        record_stream(), tenant_id, account_id
    )
...
async def test_check_for_significant_adjustments_no_change(
    ...
    new_records = [
        {"timestamp": now, "service": "EC2", "region": "us-east-1", "cost_usd": 10.0}
    ]
...
@pytest.mark.asyncio
async def test_check_for_significant_adjustments_with_audit_log(
    persistence_service, mock_db
):
    tenant_id = str(uuid4())
    account_id = str(uuid4())
    ...
    assert audit_logs[0].reason == "RESTATEMENT"


@pytest.mark.asyncio
async def test_save_summary_non_preliminary(
    persistence_service, mock_db, sample_summary
):
    ...
    with patch.object(
        persistence_service, "_check_for_significant_adjustments", AsyncMock()
    ) as mock_check:
        await persistence_service.save_summary(
            sample_summary, account_id, is_preliminary=False
        )
        assert mock_check.called


@pytest.mark.asyncio
async def test_check_for_significant_adjustments_no_existing(
    persistence_service, mock_db
):
    tenant_id = str(uuid4())
    account_id = str(uuid4())
...
@pytest.mark.asyncio
async def test_check_for_significant_adjustments_significant(
    persistence_service, mock_db
):
    tenant_id = str(uuid4())
    account_id = str(uuid4())
...
async def test_save_records_stream_batch_flush(persistence_service):
    ...
    with (
        patch.object(persistence_service, "_bulk_upsert", AsyncMock()) as mock_bulk,
        patch.object(
            persistence_service, "_check_for_significant_adjustments", AsyncMock()
        ),
    ):
...
@pytest.mark.asyncio
async def test_bulk_upsert_sqlite_updates_existing_amount_raw(
    persistence_service, mock_db
):
    mock_db.bind.url = "sqlite+aiosqlite:///:memory:"
    existing_record = MagicMock()
...
@pytest.mark.asyncio
async def test_check_for_significant_adjustments_zero_old_cost(
    persistence_service, mock_db
):
    tenant_id = str(uuid4())
    account_id = str(uuid4())
