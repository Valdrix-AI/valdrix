...
async def test_list_cur_files_manifest_dedup(cur_adapter):
    ...
    with patch.object(cur_adapter.session, "client") as mock_client:
        ...
        page = {
            "Contents": [
                {
                    "Key": "cur/2026/01/manifest.json",
                    "LastModified": datetime(2026, 1, 1),
                },
                {
                    "Key": "cur/2026/01/alt/manifest.json",
        ...
        mock_body.read.return_value = json.dumps(
            {
                "reportKeys": [
                    "cur/2026/01/report-1.parquet",
                    "cur/2026/01/report-2.parquet",
                ]
            }
        ).encode()
...
async def test_parse_cur_files_success(cur_adapter):
    ...
    with patch.object(cur_adapter.session, "client") as mock_client:
        ...
        with patch("pyarrow.parquet.read_table", return_value=mock_table):
            # 1. Without group_by
            results = await cur_adapter._parse_cur_files(
                files, start, end, group_by_service=False
            )
            ...
            # 2. With group_by
            results = await cur_adapter._parse_cur_files(
                files, start, end, group_by_service=True
            )
...
async def test_parse_cur_files_handles_file_errors(cur_adapter):
    ...
    with patch.object(cur_adapter.session, "client") as mock_client:
        ...
        with patch(
            "pyarrow.parquet.read_table", side_effect=RuntimeError("parse fail")
        ):
            results = await cur_adapter._parse_cur_files(
                files, start, end, group_by_service=False
            )
