...
class TestAttributionEngine:
    ...
    def test_match_conditions_service(self, mock_cost_record):
        ...
        # Should match
        assert (
            engine.match_conditions(mock_cost_record, {"service": "AmazonS3"}) is True
        )

        # Should not match
        assert (
            engine.match_conditions(mock_cost_record, {"service": "AmazonEC2"}) is False
        )
    ...
    def test_match_conditions_tags(self, mock_cost_record):
        ...
        assert (
            engine.match_conditions(mock_cost_record, {"tags": {"Team": "Ops"}}) is True
        )

        # Should match - multiple tags
        assert (
            engine.match_conditions(
                mock_cost_record, {"tags": {"Team": "Ops", "Environment": "Prod"}}
            )
            is True
        )
        ...
        assert (
            engine.match_conditions(mock_cost_record, {"tags": {"Team": "Dev"}})
            is False
        )

        # Should not match - missing tag
        assert (
            engine.match_conditions(mock_cost_record, {"tags": {"Project": "Main"}})
            is False
        )
    ...
    def test_match_conditions_combined(self, mock_cost_record):
        ...
        assert (
            engine.match_conditions(
                mock_cost_record,
                {"service": "AmazonS3", "region": "us-east-1", "tags": {"Team": "Ops"}},
            )
            is True
        )
