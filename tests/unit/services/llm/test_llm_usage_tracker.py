...
async def test_authorize_request_within_budget(tracker):
    ...
    with patch(
        ...
        res = await tracker.authorize_request(
            tenant_id, "openai", "gpt-4o", "hello", 1000
        )
        assert res is True
    ...
    with patch(
        ...
        with pytest.raises(BudgetExceededError):
            await tracker.authorize_request(
                tenant_id, "anthropic", "claude-3-opus", "heavy payload", 1000000
            )
...
async def test_check_budget_status_hard_limit_fail_closed(tracker):
    ...
    with patch(
        ...
        mock_check.side_effect = BudgetExceededError(
            "DB Down", details={"fail_closed": True}
        )
