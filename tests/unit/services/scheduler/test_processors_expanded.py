    ...
    async def test_process_tenant_no_connections(self, mock_db: AsyncMock) -> None:
        ...
        with patch("app.modules.governance.domain.scheduler.processors.get_settings"):
            ...
            result = await processor.process_tenant(
                mock_db, tenant, date.today(), date.today()
            )
            # Should finish early
...
class TestSavingsProcessorExpanded:
    ...
    def test_map_action_to_enum(self):
        ...
        assert (
            processor._map_action_to_enum("delete volume vol-123")
            == RemediationAction.DELETE_VOLUME
        )
        assert (
            processor._map_action_to_enum("stop instance i-123")
            == RemediationAction.STOP_INSTANCE
        )
        assert (
            processor._map_action_to_enum("terminate instance i-456")
            == RemediationAction.TERMINATE_INSTANCE
        )
        assert (
            processor._map_action_to_enum("resize instance i-789")
            == RemediationAction.RESIZE_INSTANCE
        )
        assert (
            processor._map_action_to_enum("delete snapshot snap-100")
            == RemediationAction.DELETE_SNAPSHOT
        )
        assert (
            processor._map_action_to_enum("release elastic ip eip-200")
            == RemediationAction.RELEASE_ELASTIC_IP
        )
        assert (
            processor._map_action_to_enum("stop rds instance db-300")
