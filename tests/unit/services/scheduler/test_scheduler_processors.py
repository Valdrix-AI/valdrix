...
class TestAnalysisProcessor:
    ...
    async def test_process_tenant_success(self, mock_db, mock_tenant):
        ...
        with patch("app.modules.governance.domain.scheduler.processors.AdapterFactory") as mock_factory, \
             ...
             patch("app.modules.governance.domain.scheduler.processors.LLMFactory") as mock_llm_factory, \
             patch("app.modules.governance.domain.scheduler.processors.FinOpsAnalyzer") as mock_analyzer:
            mock_adapter_instance = MagicMock()
            usage_summary = MagicMock()
            usage_summary.records = [MagicMock(amount=25.0, service="Amazon EC2")]
            usage_summary.total_cost = 25.0
            mock_adapter_instance.get_daily_costs = AsyncMock(return_value=usage_summary)
            ...
            mock_analyzer.return_value.analyze = AsyncMock(return_value={"insights": [], "recommendations": []})
...
class TestSavingsProcessor:
    ...
    async def test_process_recommendations_with_autonomous_ready(self, mock_db):
        ...
        with patch("app.modules.optimization.domain.remediation.RemediationService") as mock_service:
