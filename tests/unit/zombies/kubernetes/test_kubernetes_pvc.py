...
async def test_kubernetes_pvc_plugin_structure():
    ...
    from app.modules.optimization.adapters.kubernetes.plugins.kubernetes_pvc import (
        OrphanedPVCPlugin,
    )
...
async def test_kubernetes_pvc_scan_logic():
    ...
    pod1.spec.volumes = [
        MagicMock(persistent_volume_claim=MagicMock(claim_name="used-pvc"))
    ]
    ...
    mock_core_v1.list_persistent_volume_claim_for_all_namespaces = AsyncMock(
        return_value=mock_pvc_list
    )
    ...
    mock_api_client_instance.__aenter__ = AsyncMock(
    ...
    with patch(
        "app.modules.optimization.adapters.kubernetes.plugins.kubernetes_pvc.client.ApiClient",
        return_value=mock_api_client_instance,
    ):
        with patch(
            "app.modules.optimization.adapters.kubernetes.plugins.kubernetes_pvc.client.CoreV1Api",
            return_value=mock_core_v1,
        ):
            with patch(
                "app.modules.optimization.adapters.kubernetes.plugins.kubernetes_pvc.k8s_config.load_kube_config",
                new_callable=AsyncMock,
            ):
                zombies = await plugin.scan(
                    session=session, region="us-east-1", config={"kubeconfig": "fake"}
                )
